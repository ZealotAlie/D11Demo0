...
void Ssao::Build(D3DApp* pApp)
{
	OnSize(pApp);

	BuildFullScreenQuad(pApp->GetDevice());
	BuildOffsetVectors();
	BuildRandomVectorTexture(pApp->GetDevice());
}

void Ssao::OnSize(D3DApp* pApp)
{
	mRenderTargetWidth = pApp->GetClientWidth();
	mRenderTargetHeight = pApp->GetClientHeight();
	...
	BuildFrustumFarCorners(pApp->GetCamera().GetFovY(), pApp->GetCamera().GetFarZ());
    BuildTextureViews(pApp->GetDevice());
}
 
void Ssao::SetNormalDepthRenderTarget(ID3D11DeviceContext* pContext,ID3D11DepthStencilView* dsv)
{
...
void Ssao::ComputeSsao(D3DApp* pApp)
{
	...
	pContext->OMSetRenderTargets(1, renderTargets, 0);
	pContext->ClearRenderTargetView(mAmbientRTV0, reinterpret_cast<const float*>(&Colors::Black));
	pContext->RSSetViewports(1, &mAmbientMapViewport);
	...
	SsaoEffect::Ptr()->SetViewToTexSpace(PT);
	SsaoEffect::Ptr()->SetOffsetVectors(mOffsets);
	SsaoEffect::Ptr()->SetFrustumCorners(mFrustumFarCorner);
	SsaoEffect::Ptr()->SetNormalDepthMap(mNormalDepthSRV);
	SsaoEffect::Ptr()->SetRandomVecMap(mRandomVectorSRV);
	...
	pContext->IASetVertexBuffers(0, 1, &mScreenQuadVB, &stride, &offset);
	pContext->IASetIndexBuffer(mScreenQuadIB, DXGI_FORMAT_R16_UINT, 0);
	
	ID3DX11EffectTechnique* tech = SsaoEffect::Ptr()->SsaoTech;
...
void Ssao::BlurAmbientMap(ID3D11ShaderResourceView* inputSRV, ID3D11RenderTargetView* outputRTV, bool horzBlur)
{
	...
	ID3D11DeviceContext* pContext = D3DApp::GetInstance()->GetContext();

	pContext->OMSetRenderTargets(1, renderTargets, 0);
	pContext->ClearRenderTargetView(outputRTV, reinterpret_cast<const float*>(&Colors::Black));
	pContext->RSSetViewports(1, &mAmbientMapViewport);

	SsaoBlurEffect::Ptr()->SetTexelWidth(1.0f / mAmbientMapViewport.Width );
	SsaoBlurEffect::Ptr()->SetTexelHeight(1.0f / mAmbientMapViewport.Height );
	SsaoBlurEffect::Ptr()->SetNormalDepthMap(mNormalDepthSRV);
	SsaoBlurEffect::Ptr()->SetInputImage(inputSRV);

	ID3DX11EffectTechnique* tech;
	if(horzBlur)
	{
		tech = SsaoBlurEffect::Ptr()->HorzBlurTech;
	}
	else
	{
		tech = SsaoBlurEffect::Ptr()->VertBlurTech;
	}
...
void Ssao::BuildFullScreenQuad(ID3D11Device *pDevice)
{
	Basic32 v[4];
...
void Ssao::BuildTextureViews(ID3D11Device *pDevice)
{
	ReleaseTextureViews();
	...
	HR(pDevice->CreateTexture2D(&texDesc, 0, &normalDepthTex));
	HR(pDevice->CreateShaderResourceView(normalDepthTex, 0, &mNormalDepthSRV));
	HR(pDevice->CreateRenderTargetView(normalDepthTex, 0, &mNormalDepthRTV));
	...
	HR(pDevice->CreateShaderResourceView(ambientTex0, 0, &mAmbientSRV0));
	HR(pDevice->CreateRenderTargetView(ambientTex0, 0, &mAmbientRTV0));
	
	ID3D11Texture2D* ambientTex1 = 0;
	HR(pDevice->CreateTexture2D(&texDesc, 0, &ambientTex1));
	HR(pDevice->CreateShaderResourceView(ambientTex1, 0, &mAmbientSRV1));
	HR(pDevice->CreateRenderTargetView(ambientTex1, 0, &mAmbientRTV1));
...
void Ssao::BuildRandomVectorTexture(ID3D11Device *pDevice)
{
	D3D11_TEXTURE2D_DESC texDesc;
	texDesc.Width = 256;
	texDesc.Height = 256;
	...
	HR(pDevice->CreateShaderResourceView(tex, 0, &mRandomVectorSRV));
