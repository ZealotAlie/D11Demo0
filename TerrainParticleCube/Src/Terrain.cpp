...
void Terrain::Draw(ID3D11DeviceContext* dc, const Camera& cam, DirectionalLight lights[3])
{
	...
	dc->IASetInputLayout(TerrainVertex::GetInputLayout());
	...
	TerrainEffect::Ptr()->SetViewProj(viewProj);
	TerrainEffect::Ptr()->SetEyePosW(cam.GetPosition());
	TerrainEffect::Ptr()->SetDirLights(lights);
	TerrainEffect::Ptr()->SetFogColor(Colors::Silver);
	TerrainEffect::Ptr()->SetFogStart(15.0f);
	TerrainEffect::Ptr()->SetFogRange(175.0f);
	TerrainEffect::Ptr()->SetMinDist(20.0f);
	TerrainEffect::Ptr()->SetMaxDist(500.0f);
	TerrainEffect::Ptr()->SetMinTess(0.0f);
	TerrainEffect::Ptr()->SetMaxTess(6.0f);
	TerrainEffect::Ptr()->SetTexelCellSpaceU(1.0f / mInfo.HeightmapWidth);
	TerrainEffect::Ptr()->SetTexelCellSpaceV(1.0f / mInfo.HeightmapHeight);
	TerrainEffect::Ptr()->SetWorldCellSpace(mInfo.CellSpacing);
	TerrainEffect::Ptr()->SetWorldFrustumPlanes(worldPlanes);
	
	TerrainEffect::Ptr()->SetLayerMapArray(mLayerMapArraySRV);
	TerrainEffect::Ptr()->SetBlendMap(mBlendMapSRV);
	TerrainEffect::Ptr()->SetHeightMap(mHeightMapSRV);

	TerrainEffect::Ptr()->SetMaterial(mMat);

	ID3DX11EffectTechnique* tech = TerrainEffect::Ptr()->Light1Tech;
