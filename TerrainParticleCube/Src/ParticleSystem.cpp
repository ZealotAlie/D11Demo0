...
void ParticleSystem::Draw(ID3D11DeviceContext* dc, const Camera& cam)
{
	...
	// Set IA stage.
	//
	dc->IASetInputLayout(ParticleData::GetInputLayout());
...
void ParticleSystem::BuildVB(ID3D11Device* device)
{
	...
	vbd.ByteWidth = sizeof(ParticleData) * 1;
	...
	ParticleData p;
	ZeroMemory(&p, sizeof(ParticleData));
	p.Age  = 0.0f;
	p.Type = 0; 
	...
	//
	vbd.ByteWidth = sizeof(ParticleData) * mMaxParticles;
